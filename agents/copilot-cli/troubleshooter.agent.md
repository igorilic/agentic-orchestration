...
3. **Azure Log Analytics** — exceptions, failed requests, traces
   (`az monitor log-analytics query --workspace <GUID> --analytics-query "..."`)
4. **Codebase** — recent changes, source of failing code

## Workflow
1. Understand (read ticket)
2. Gather (ArgoCD + Log Analytics + git log)
